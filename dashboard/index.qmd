...
function toggleParams() {
  ...
  // Use computedStyle to detect actual visibility — more reliable than classList check
  // alone, since both class and inline style can cause display:none.
  const isHidden = window.getComputedStyle(sidebar).display === 'none';

  if (isHidden) {
    sidebar.classList.remove('params-hidden');
    sidebar.style.removeProperty('display');          // explicit removal, not '' assignment
    ...
    sidebar.classList.add('params-hidden');           // CSS class has display:none !important
...
<!-- z-index 10001: must be above Hypothes.is badge (~9999) so clicks reach our buttons -->
<div id="view-controls" style="position: fixed; bottom: 20px; right: 52px; z-index: 10001; display: flex; flex-direction: column; gap: 6px; background: white; border: 1px solid #ccc; border-radius: 8px; padding: 10px; box-shadow: 0 2px 12px rgba(0,0,0,0.15); font-size: 12px; max-width: 160px;">
...
function simulate(n, seed, params) {
  ...
  return {
    ...
    // Process mode realized shares
    mode_labels_samples: mode_labels,
...
//| echo: false
{
  const ml = results.mode_labels_samples;
  const n = ml.length;
  const isOverride = ml[0] === "override";
  const pct_fb = isOverride ? null : (ml.filter(m => m === "fedbatch").length / n * 100).toFixed(0);
  const pct_pf = isOverride ? null : (ml.filter(m => m === "perfusion").length / n * 100).toFixed(0);
  const pct_ct = isOverride ? null : (ml.filter(m => m === "continuous").length / n * 100).toFixed(0);
