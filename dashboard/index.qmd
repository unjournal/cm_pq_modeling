...
function initOjsTooltips() {
  ...
  document.querySelectorAll('.panel-sidebar abbr[title], .panel-fill abbr[title]').forEach(function(el) {
    if (!el._tippy && el.getAttribute('title')) {
      tippy(el, { content: el.getAttribute('title'), placement: 'bottom', maxWidth: 340, allowHTML: false });
    }
  });
}
...
```{=html}
<div style="display: flex; gap: 6px; margin-bottom: 0.75rem; position: sticky; top: 0; background: white; padding: 0.5rem 0 0.5rem; border-bottom: 1px solid #eee; z-index: 5; margin-top: -0.5rem;">
