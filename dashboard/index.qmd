...
// Wide sidebar mode: expands the parameters column to ~60% of the page width
// so users can read the detailed explanations and set parameters comfortably,
// then click again to return to the narrow default.
function toggleWideSidebar() {
  ...
  if (body.classList.contains('sidebar-wide')) {
    ...
    if (!document.getElementById('sidebar-wide-style')) {
      ...
      // Override Quarto's panel grid so the sidebar takes ~60vw
      s.textContent = `
        ...
        body.sidebar-wide .page-columns,
        body.sidebar-wide #quarto-content > .page-columns {
          grid-template-columns: 60vw 1fr !important;
        }
      `;
...
<!-- Fixed floating view controls — LOWER LEFT to avoid Hypothes.is sidebar on the right -->
<div id="view-controls" style="position: fixed; bottom: 20px; left: 20px; z-index: 10001; display: flex; flex-direction: column; gap: 6px; background: white; border: 1px solid #ccc; border-radius: 8px; padding: 10px; box-shadow: 0 2px 12px rgba(0,0,0,0.15); font-size: 12px; max-width: 165px;">
...
<div style="display: flex; gap: 6px; margin-bottom: 0.75rem;">
  <button id="toggle-params" onclick="toggleParams()" style="flex: 1; padding: 0.5rem; font-size: 0.85rem; cursor: pointer; border: 1px solid #5a7a5a; border-radius: 6px; background: #e8f4e8; color: #2d4a2d; font-weight: 500;">
    ◀ Hide Parameters
  </button>
  <button id="toggle-wide-params" onclick="toggleWideSidebar()" title="Expand sidebar to read explanations in more detail" style="padding: 0.5rem 0.6rem; font-size: 0.85rem; cursor: pointer; border: 1px solid #3498db; border-radius: 6px; background: #f0f8ff; color: #1a5276; font-weight: 500; white-space: nowrap;">
    ⟺ Wide view
  </button>
</div>
```
...
//| echo: false
{
  ...
  return html`<div style="font-size:0.84em; color:#555; margin: -0.2rem 0 0.4rem 0;">
    Effective: Fed-batch <strong>${fb}%</strong> · Perfusion <strong>${pf}%</strong> · Continuous <strong>${ct}%</strong>
  </div>`;
}
```
