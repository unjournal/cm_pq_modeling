...
```{ojs}
//| echo: false
// URL state helpers: let any slider's default value pick up a value from
// ?key=val in the page URL, so a link captures the full parameter set.
// Parse once on page load; a separate reactive cell (below the sliders)
// writes back to the URL when viewof values change.
urlParams = {
...
```{ojs}
//| echo: false
// URL state writer: serialize every viewof value into ?key=val pairs and
// update the URL via history.replaceState (debounced so dragging a slider
// doesn't spam the history API). The writer depends on every viewof name
// below so OJS re-runs it whenever any input changes. Reads nothing from
// urlParams, so there's no feedback loop with the initial-value readers.
{
  const state = {
  ...
  window._urlWriteTimer = setTimeout(() => {
    try {
      history.replaceState(null, "", "?" + usp.toString() + location.hash);
    } catch (e) {
