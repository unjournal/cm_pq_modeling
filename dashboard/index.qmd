...
<strong>Simplified view:</strong> Less pivotal parameters (plant capacity, uptime, financing costs, media turnover) are set to reasonable defaults.
...
//| echo: false
{
  ...
  //   • L/kg (volume)     — deterministic function of density × turnover
  ...
  const params = [
    ...
    {name: "Media Turnover",                     data: results.media_turnover_samples, kind: "primitive"},
...
- **L/kg (volume)** was a pure deterministic function of Cell Density × Media Turnover — not an independent driver, just a third lens on the same two variables.
...
**What the other bars mean.** The mixture bars (Media $/L, GF Price, GF Quantity) bundle the regime switch with the within-regime lognormal noise — the swing is the combined effect. The primitive bars (Cell Density, Media Turnover, Plant Capacity, Utilization Rate) are sampled close to independently, so their swings are close to honest marginal effects over the joint distribution.
...
This model was largely AI-generated. We have grounded key parameter ranges in published TEAs and recently revised the pharma-grade media cost range downward (from $1-4/L to $0.50-2.50/L) based on GFI's 2025 amino acid supplier data. However, **we have not yet systematically verified that every hardcoded range traces to a specific source.** Some ranges (particularly for growth factor quantities, media turnover, and cycle days) may reflect AI-synthesized estimates rather than direct citations. This is one reason we are seeking expert review.
:::
