...
```{ojs}
//| echo: false
viewof cdmo_mode = Inputs.toggle({label: "CDMO / Contract Manufacturing mode", value: urlBool("cdmo_mode", false)})
```

```{ojs}
//| echo: false
html`<div style="display: ${cdmo_mode ? 'block' : 'none'}; padding: 0.6rem 0.8rem; background: #fef9e7; border-left: 3px solid #f39c12; font-size: 0.84em; margin: 0.3rem 0 0.6rem 0; line-height: 1.5;">
<strong>CDMO mode on.</strong> CAPEX and Plant Overhead OPEX are replaced by a <em>contract toll fee</em> — what a CDMO charges per kg to cover their own capital, labour, and margin. Your direct costs remain VOC (media, growth factors, other variable). A <a href="#in-house-vs-cdmo-comparison">side-by-side comparison</a> appears in the results.
</div>`
```
...
<details><summary>What is CDMO mode? (click to expand)</summary>

A **CDMO (Contract Development and Manufacturing Organization)** is a company that manufactures products on behalf of other companies. In the cultured meat context, a CDMO would:

- Own and operate the bioreactors and facility
- Charge the cultured meat brand a *per-kg toll fee* for production
- Allow the brand to avoid direct CAPEX and fixed overhead

**What changes in the model:**

| Cost component | In-House | CDMO mode |
|----------------|----------|-----------|
| Media (VOC) | Your cost | Your cost |
| Growth factors (VOC) | Your cost | Your cost |
| CAPEX (bioreactors, facility) | Your cost | → CDMO toll |
| Plant overhead (labour, maintenance) | Your cost | → CDMO toll |
| Downstream (optional) | Your cost | Your cost |

**CDMO toll range (default p5=$4, p95=$40/kg):**

This range attempts to cover the uncertainty about what a food-grade CDMO would charge for cultivated meat production by 2036:

| Scenario | Toll ($/kg) | Basis |
|----------|------------|-------|
| Optimistic (p5) | ~$4 | Efficient food-grade CDMO with high multi-client utilization |
| Typical | ~$12 | CDMO serving multiple clients, food-grade equipment, ~30% margin |
| Pessimistic (p95) | ~$40 | Pharma-adjacent CDMO with low utilization and high margin |

For context: current pharma CDMOs charge ~$50–500/L-day bioreactor time; at 50 g/L and a 3-day cycle, that's $1,000–30,000/kg — far too expensive. The default range assumes a future food-grade CDMO sector has developed alongside the industry.

**When would CDMO be preferable to in-house?**

- Early-stage companies that want market entry without construction risk
- If CDMO scale economies across multiple clients reduce per-unit CAPEX
- When the brand's competitive advantage is recipe/branding, not manufacturing

**When might in-house be preferable?**

- At large scale where you capture the economies yourself
- When process IP is a competitive advantage you can't share
- When long-run in-house costs fall below the CDMO margin

The comparison chart in the results section shows when each mode produces lower costs under the current parameters.
</details>
