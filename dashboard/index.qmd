...

{
  ...
  const plots = components.map(comp => {
    ...
    return Plot.plot({
      ...
      title: `${comp.name}: $${formatCost(p50)} (90% CI: ${formatCost(p5)} – ${formatCost(p95)})`
    });
  });

  return html`<div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 1rem; margin: 1rem 0;">
    ${plots.map(p => html`<div style="font-size: 0.9em;">${p}</div>`)}
  </div>`;
}
```
...
::: {.callout-tip}
**Code references:** [JavaScript source (this page)](#full-source-code) | [Python reference](https://github.com/unjournal/cm_pq_modeling/blob/main/dashboard/model.py)

*Note: An older [Squiggle model](https://squigglehub.org/models/Unjournal-modelingPQs/cultured_meat_improved_by_squiggle_improve) exists but uses different parameters. This dashboard is the current version.*
:::
...
This produces the histograms and probability thresholds shown above.

::: {.callout-note collapse="true"}
## Full Source Code {#full-source-code}

The complete JavaScript implementation is embedded in this page. Key functions:
...
View the page source or see the [Python reference implementation](https://github.com/unjournal/cm_pq_modeling/blob/main/dashboard/model.py) for details.
:::
