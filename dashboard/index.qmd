...
function simulate(n, seed, params) {
  ...
  // Total unit cost
  const unit_cost = add(add(voc, capex_perkg), fixed_perkg);
...
viewof p_hydro = Inputs.range([0.1, 0.9], {
  value: 0.6, step: 0.05,
  label: "P(Hydrolysates)"
})
```
<details><summary>What is this?</summary>
Hydrolysates are cheap plant-protein digests that can replace expensive pharmaceutical-grade amino acids. Reduces media cost by ~70%.
</details>
...
viewof p_foodgrade = Inputs.range([0.1, 0.9], {
  ...
  label: "P(Food-grade micros)"
})
```
<details><summary>What is this?</summary>
Food-grade micronutrients (vitamins, minerals) are much cheaper than pharma-grade. Reduces micronutrient cost by ~90%.
</details>
...
viewof p_recfactors = Inputs.range([0.1, 0.9], {
  value: 0.5, step: 0.05,
  label: "P(Cheap recombinant factors)"
})
```
<details><summary>What is this?</summary>
Growth factors (proteins signaling cell growth) are currently very expensive. Recombinant production at scale could reduce costs 100-1000x.
</details>
...
//| echo: false
{
  ...
  return Plot.plot({
    ...
    title: "Projected 2036 Cultured Meat Production Cost Distribution"
  })
}
```
...
<div class="grid" style="grid-template-columns: repeat(4, 1fr); gap: 1rem; margin: 2rem 0;">
