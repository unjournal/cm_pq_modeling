...
::: {.callout-note collapse="true"}
## From workshop evidence to model revision
The May 8, 2026 workshop is complete. Its outputs now form a linked evidence-to-model workflow: the summary records the technical arguments, the beliefs analysis shows the forecast spread, the demand bridge connects factory-gate cost to an adoption model, and the proposed modeling hack turns the remaining disagreements into testable model changes.

**[Workshop summary →](https://uj-cm-workshop.netlify.app/summary.html)** · **[Public beliefs form →](https://uj-cm-workshop.netlify.app/beliefs.html)** · **[Cost → demand bridge →](demand.qmd)** · **[Modeling hack proposal →](https://uj-cm-workshop.netlify.app/modeling-hack.html)**

The named beliefs-analysis dashboard remains an internal review artifact and is intentionally not linked from this public site; a redacted public synthesis should be produced separately.
:::
...
// Re-initialise Tippy tooltips for abbr[title] and span[title] elements added by OJS.
// OJS renders after Quarto's DOMContentLoaded Tippy pass, so elements inside OJS
// html`` templates are missed on first pass. Span elements (used for ⓘ inline notes
// in the main content area) are also included — native browser title tooltips clip
// long text and are unreliable across browsers.
function initOjsTooltips() {
  if (typeof tippy === 'undefined') return;
  document.querySelectorAll(
    '.panel-sidebar abbr[title], .panel-fill abbr[title], .panel-sidebar span[title], .panel-fill span[title]'
  ).forEach(function(el) {
    const t = el.getAttribute('title');
    if (!el._tippy && t) {
      el.removeAttribute('title');  // prevent duplicate native browser tooltip
      tippy(el, { content: t, placement: 'bottom', maxWidth: 500, allowHTML: false });
    }
  });
}
setTimeout(initOjsTooltips, 2500);  // first pass after OJS settles
// Re-run whenever sidebar OR main content changes (OJS re-renders on slider change)
document.addEventListener('DOMContentLoaded', function() {
  var obs = new MutationObserver(function() { initOjsTooltips(); });
  var sidebar = document.querySelector('.panel-sidebar');
  var fill = document.querySelector('.panel-fill');
  if (sidebar) obs.observe(sidebar, { childList: true, subtree: true });
  if (fill) obs.observe(fill, { childList: true, subtree: true });
});
...
<div id="view-controls" style="position: fixed; bottom: 90px; right: 20px; z-index: 10001; background: white; border: 1px solid #ccc; border-radius: 8px; box-shadow: 0 2px 12px rgba(0,0,0,0.15); font-size: 12px; min-width: 145px; overflow: hidden;">
  ...
  <div id="view-controls-content" style="display: none; flex-direction: column; gap: 5px; padding: 8px 9px;">
...
In "good worlds" for cultured chicken (high maturity), multiple things improve together. This makes favorable assumptions co-occur more often. Technical success with difficult financing remains possible and should not be ruled out.
...
These inputs are fixed for a shorter interface, not because their importance is guaranteed to be small. Inspect the background assumptions and use Advanced to vary them. The association chart ranks conditional-mean differences under the selected scenario; it does not measure shares of variance.
...
viewof bundled_media = Inputs.toggle({
  label: html`Bundled media pricing (TEA-comparable) <abbr style="cursor:help;text-decoration:underline dotted;font-size:0.85em;color:#888;" title="When ON: basal media + growth factors + supplemental proteins are replaced by a single 'complete media' $/L figure, matching the convention used in research-grade published TEAs (Humbird 2021 etc.). Default OFF = separable accounting, which is better for reasoning about future cost reduction.">(?)</abbr>`,
...
<strong>Bundled mode:</strong> Complete media $/L includes basal nutrients, growth factors, and supplemental proteins. Separate protein costs are zero; a separate GF override is ignored.
...
```{ojs}
//| echo: false
viewof bundled_media_p5 = Inputs.range([5, 200], {
...
A synthetic latent variable (0 = nascent, 1 = mature) that correlates technology adoption, reactor costs, and financing. **It is not calibrated to observed historical trajectories and does not encode an annual learning rate.** The slider is a scenario index: it represents your judgment about how many favorable conditions co-occur, not a measured pace of progress.

| Slider | Rough scenario interpretation |
|---|---|
| 0.1–0.2 | Industry largely stalls; few process improvements reach commercial use and financing remains difficult |
| 0.3–0.4 | Some technical progress, but major input, scale-up, or financing constraints remain |
| **0.5** | **Neutral coupling assumption**; the model uses the base technology probabilities without claiming this matches a historical trend |
| 0.6–0.7 | Several favorable technical and supply-chain developments co-occur |
| 0.8–0.9 | Strongly favorable bundle; most modeled pathways succeed and financing conditions improve |

Recent evidence should update the **individual parameter distributions**, not be translated into a maturity learning rate. For example, GFI's [2026 industry report](https://gfi.org/resource/cultivated-meat-seafood-and-ingredients-state-of-the-industry/) records both tighter funding and 20,000-L production progress, while the FDA's [February 2026 inventory](https://hfpappexternal.fda.gov/scripts/fdcc/index.cfm?set=animalcellculturefoods) lists five completed cultured-cell food consultations. Those signals point in different directions and should not be compressed into a single estimated trend.
...
Probability that the selected hypothetical plant uses each cost-reducing technology by the projection year. These are scenario probabilities, not industry production shares.
...
**Note on \$/L vs \$/kg output:** The model uses \$/L as an internal parameter, but the economically meaningful unit is **\$/kg of cell biomass produced** — which equals (\$/L) × (L consumed per kg output). L per kg = 1000/density(g/L) × media_turnover_multiplier. In an illustrative scenario (60 g/L density, 1.5× multiplier), this is ~25 L/kg: a \$0.70/L hydrolysate medium costs ~\$17.5/kg output; a \$1.25/L pharma-grade medium costs ~\$31/kg. At 200 g/L perfusion with 1.0× turnover (~5 L/kg), the same formulations cost \$3.5/kg and \$6.25/kg respectively — illustrating the inverse-density relationship when all other inputs are fixed; both density and price have unit elasticity in this formula. The "Media" bar in the cost breakdown chart already shows this \$/kg result directly.
...
**Why 75% baseline?** This is a judgmental scenario default, not an empirically estimated adoption rate. [Pasitka et al. (2024)](https://www.nature.com/articles/s43016-024-01022-w) demonstrated a low-cost animal-component-free medium in one chicken-cell process, while [Mi et al. (2025)](https://doi.org/10.1016/j.cej.2025.164496) found that selected plant hydrolysates reduced serum use by up to 70% in a porcine-cell prototype. These are meaningful feasibility results, but they do not establish full substitution, lot-to-lot consistency, or commercial performance across cell lines and large reactors.

**How pivotal is this parameter?** The unresolved questions are technical, operational, and regulatory: amino-acid completeness, supplementation needs, batch variability, quality control, performance at scale, residuals, and product-specific safety review. Current US and UK guidance focuses on process controls, hazards, and evidence for the submitted product; we have not found support for the stronger claim that regulators generally require pharmaceutical-grade ingredients. Treat 75% as a scenario to stress-test, not a consensus forecast.
...
viewof gf_progress = Inputs.range([0, 100], {
  ...
  label: html`GF price progress index for ${target_year} (%)`
})
```
...
//| echo: false
{
  const ranges = costModel.gfPriceRanges(gf_progress);
  return html`<p title="Reports 5 and 6 requested visible control semantics. These are the engine's existing assumed price distributions, not newly fitted evidence."><strong>Current GF price ranges (p5–p95):</strong> cheap regime $${ranges.cheap[0].toFixed(2)}–$${ranges.cheap[1].toFixed(2)}/g; expensive regime $${ranges.expensive[0].toFixed(2)}–$${ranges.expensive[1].toFixed(2)}/g. Adoption selects the regime; progress shifts both ranges. Direct GF-cost priors and complete medium replace this calculation.</p>`;
}
```
...
| Breakthrough achieved (≈3× usage reduction) | 0.0005 – 0.002 | $10 – $1,000 | Endpoint products $0.005–2 (not a cost credible interval) |
...
**Albumin, transferrin, and insulin** are media additives — proteins grouped separately here for cost accounting (insulin also has signaling functions). They are widely used in serum-free cell culture but rarely modeled explicitly in CM TEAs (usually bundled into "other VOC" or media cost).
...
**Workshop discussion:** This was one of the key cruxes for the [May 8, 2026 CM workshop](https://uj-cm-workshop.netlify.app/) — what bioreactor scale is realistic, and what does it cost?
</details>
...
**Advanced: <span title="Net fresh media consumed per kg of cells, divided by the nominal (1000/density) reactor-fill volume. 1 = batch; >1 = perfusion; <1 = recycling / fed-batch / explicit medium recovery.">Media-use multiplier (×)</span>**
...
**Range interpretation:** The manual override uses p5–p95 0.5–3.0; default sampling instead uses process-specific ranges. These are lognormal percentiles, not hard bounds. Values below one need explicit medium recovery and consistently priced replacement nutrients. For example, 8 L/kg at 90 g/L implies a multiplier of 0.72, while the same medium use at 200 g/L implies 1.6. A lower multiplier is not the only way to achieve a particular L/kg value.
...
// whenever any input changes. Reads nothing from urlParams.
{
  ...
  const state = {
    ...structure_settings,
    ...Object.fromEntries(Object.entries(expert_priors_adv).map(([k, v]) => [`ep_${k}`, v])),
...
viewof expert_priors_adv = modelControls.expertPriorControl(new URLSearchParams(urlParams))
viewof structure_settings = modelControls.structuralControl(new URLSearchParams(urlParams))
```
...
simParams = {
  const adjustedMaturity = costModel.maturityForYear(maturity, target_year);
  return {
    ...structure_settings,
    target_year,
...
```{ojs}
//| echo: false
html`<div role="status">${results.warnings.map(w => html`<p><strong>Scenario note:</strong> ${w}</p>`)}</div>`
```
...
// N=30,000 balances browser responsiveness and sampling precision.
// Repeat-seed variability is reported by scripts/model-audit.mjs.
...
<strong>What these numbers represent:</strong> Simulated manufacturing cost per kg of cultured chicken cell biomass (<span title="Intended basis: wet cell biomass around the harvest gate. The calculation treats the cell-density inputs as grams of wet biomass per litre; it does not separately apply or standardize an 80% hydration factor. Washing, dewatering, residual media, dry-matter content, and protein content can therefore differ across source studies. Includes media, growth factors, annualized bioreactor capital, plant overhead, and utilities. Excludes product formulation, texturization, packaging, distribution, and retail margins." style="text-decoration: underline dotted; cursor: help;">wet-biomass basis &#9432;</span>) in ${target_year}, based on ${stats.n.toLocaleString()} Monte Carlo simulations. This is a factory-gate ingredient-cost estimate, not a consumer-product price. <a href="compare.html" style="font-size: 0.9em;">[Compare to published TEAs →]</a>
<br><br>
<strong><span title="UPSIDE Foods' chicken cutlet is a blend of cultured chicken cells and plant-based ingredients. SuperMeat's chicken burger used ~30% cultured cells. The GFI State of the Industry 2024 report notes that 'hybrid products combining cultivated and plant-based ingredients are the most likely near-term path to market.' Eat Just/GOOD Meat's Singapore-approved product uses cultured chicken in a plant-protein matrix." style="text-decoration: underline dotted; cursor: help;">Pure cells vs. consumer products:</span></strong> Most cultivated meat products on the market or in development are <em>hybrid products</em> — blending a fraction of cultured cells with plant-based or mycoprotein ingredients. A product with (say) 20% cultured cells and 80% plant-based filler at $3/kg would have a blended ingredient cost far below the pure-cell cost shown here. Assessing retail parity still requires a matched product and the remaining value-chain costs.
<br><br>
<strong>Three cautions before using the output:</strong> (1) source costs are not yet normalized to one real-dollar base year; (2) the CM_01 beliefs question conditions on large-scale commercial production being achieved, while this simulation also contains low-maturity draws and has no separate probability-of-commercialization stage; and (3) factory-gate biomass cost cannot be compared directly with retail chicken prices without matching inclusion rate, solids/protein basis, downstream yield, other ingredients, packaging, distribution, margin, geography, and currency year. The simple blended-product arithmetic below is an ingredient-cost scenario, not a price-parity result.
</div>`
```
...
// closure — the cell re-runs on every results change, which is fine.
{
  ...
  function downloadCSV() {
    ...
    const metaCols = ["model_version", "seed", "sample_count", "effective_params_json"];
    const quote = v => '"' + String(v).replaceAll('"', '""') + '"';
    const meta = [results.model_version, results.seed, results.sample_count].map(quote);
    const rows = [[...cols, ...metaCols].join(",")];
    const n = results.unit_cost.length;
    for (let i = 0; i < n; i++) {
      // The full configuration appears once, on the first sample row.
      rows.push([...cols.map(c => results[c][i].toFixed(4)), ...meta, i === 0 ? quote(JSON.stringify(results.effective_params)) : '""'].join(","));
    }
  ...
  function downloadJSON() {
    const config = {
      model_version: costModel.MODEL_VERSION,
      review_provenance: results.provenance,
      structural_comparison: structural_comparison_rows,
      effective_simulation_parameters: results.effective_params,
      ...
      notes: {
        units: "USD per kg of cultured chicken cell biomass on the model's wet-biomass basis",
        currency_basis: "Mixed source-year USD; not yet normalized to a common real-dollar year",
        conditioning: "Does not separately model P(large-scale commercialization); not a clean conditional-on-success forecast",
        simulation_seed: 42,
...
- **Dark green dotted**: $10/kg reference (ingredient cost only; excludes retail costs)
...
//| echo: false
{
  ...
  const grid = `<div class="grid" style="grid-template-columns: repeat(4, 1fr); gap: 1rem; margin: 1.5rem 0;">
    ${card(10,  stats.prob_10,  'illustrative manufacturing-cost threshold',  '#27ae60', stats.bprob_10,  bs, fc)}
    ${card(25,  stats.prob_25,  'illustrative manufacturing-cost threshold',              '#3498db', stats.bprob_25,  bs, fc)}
    ${card(50,  stats.prob_50,  'illustrative manufacturing-cost threshold',                         '#f39c12', stats.bprob_50,  bs, fc)}
    ${card(100, stats.prob_100, 'illustrative manufacturing-cost threshold',               '#e74c3c', stats.bprob_100, bs, fc)}
  </div>`;
  ...
  const blendRow = include_blending ? `
    <p style="font-size:0.88em; color:#1a5276; margin:0.5rem 0 0.3rem; font-weight:500;">
      Blended product (${Math.round(bs*100)}% CM + ${Math.round((1-bs)*100)}% filler at $${fc}/kg) — ingredient-cost thresholds:
    </p>
    <div class="grid" style="grid-template-columns: repeat(3, 1fr); gap: 0.75rem; margin-bottom: 1.5rem;">
      <div class="card" style="border: 2px solid ${stats.bprob_5  > 20 ? '#27ae60' : '#ddd'}; padding: 0.8rem; border-radius: 8px; text-align: center;">
        ...
        <small>ingredient cost only; excludes retail costs</small>
      </div>
    ...
    <p style="font-size:0.82em; color:#777; margin-bottom:1rem; font-style:italic;">
      Blending reduces the cultured-cell contribution per kg of product. The result excludes formulation losses, processing, packaging, distribution, and retail margins.
    </p>` : '';
...
//| echo: false
{
  const mediaLabel = bundled_media ? "Complete Media (all proteins included)" : "Media (incl. basal micros)";
  const allComponents = [
...
{
  const allComponents = [
    {name: bundled_media ? "Complete Media (all proteins included)" : "Media (incl. basal micros)", data: results.cost_media, color: "#27ae60"},
...
### Association under the current uncertainty model {#sensitivity-analysis-tornado-chart}

Which parameters have the most impact on the final cost? Each bar shows the **dollar swing** in mean unit cost between simulations where the parameter is in its top 10% versus its bottom 10%. Larger bars indicate stronger associations under the selected assumptions.
...
//| echo: false
{
  ...
  const params = [
    ...
    {name: "Effective production time (days)", data: results.cycle_days_samples, kind: "primitive"},
    {name: "Utilization Rate",                   data: results.uptime_samples,         kind: "primitive"}
  ];

  // Exclude replaced or disabled inputs rather than ranking irrelevant random draws.
  const mediaOverride = simParams.ep_media_p10 != null;
  const gfOverride = simParams.ep_gf_p10 != null;
  const capitalActive = !simParams.cdmo_mode && simParams.include_capex;
  const mediaVolumeActive = !mediaOverride || (simParams.gf_dose_model === "media_linked" && !gfOverride && !simParams.bundled_media);
  const activeParams = params.filter(p => {
    if (p.name.startsWith("Industry Maturity")) return simParams.dependence_mode === "shared";
    if (p.name.startsWith("Media-use")) return simParams.media_use_model === "density" && mediaVolumeActive;
    if (p.name.startsWith("Media $/L")) return !mediaOverride;
    if (p.name.startsWith("GF ")) return !simParams.bundled_media && !gfOverride;
    if (p.name.startsWith("WACC") || p.name.startsWith("Asset Life") || p.name.startsWith("Effective production")) return capitalActive;
    if (p.name.startsWith("Cell Density")) return (simParams.media_use_model === "density" && mediaVolumeActive) || capitalActive;
    if (p.name.startsWith("Plant Capacity") || p.name.startsWith("Utilization")) return capitalActive || (!simParams.cdmo_mode && simParams.include_fixed_opex);
    return true;
  });
  if (simParams.media_use_model === "direct" && mediaVolumeActive) activeParams.push({name: "Fresh media (L/kg)", data: results.L_per_kg_samples, kind: "primitive"});
  if (mediaOverride) activeParams.push({name: "Media cost override ($/kg)", data: results.cost_media, kind: "override"});
  if (gfOverride && !simParams.bundled_media) activeParams.push({name: "GF cost override ($/kg)", data: results.cost_recf, kind: "override"});

  // Compute signed swings: positive = high param → higher cost (red); negative = high param → lower cost (green)
  const swingsRaw = activeParams.map(p => ({
    name: p.name,
    kind: p.kind,
  ...
  const tornadoPlot = Plot.plot({
    ...
    marginRight: 120,
    x: {
      label: "Dollar swing in mean unit cost ($/kg) — larger bar = stronger cost association",
      domain: [0, maxAbs + pad],
      grid: true,
      labelOffset: 40,
  ...
  return html`<div style="font-size: 1em;">
    <div style="font-weight: normal; font-size: 1.05em; margin-bottom: 0.5rem; color: #333;">Which sampled inputs are associated with higher or lower costs?</div>
    ${tornadoPlot}
  </div>`;
}
```

*Red bars: high values occur with higher costs. Green bars: high values occur with lower costs. Bar length = conditional-mean dollar swing between top 10% vs. bottom 10% of that parameter's distribution across the 30,000 Monte Carlo draws. This is an association, not an intervention effect or variance decomposition. Correlated inputs overlap; bars should not be summed or interpreted as research value. Industry Maturity is a latent variable whose bar captures the bundled effect of correlated improvements.* [Full methodology →](docs.html#sensitivity-analysis-dollar-swing-metric){target="_blank"}

### Structural comparison {#structural-comparison}

These runs keep your other inputs and seed (42) fixed. Each row changes one structural choice relative to your selected scenario. Fresh-media endpoints and GF concentration use the values in **Structural stress tests**. Comparisons reflect those provisional inputs as well as model form; they are not empirical validation or a variance decomposition. Overrides may make a comparison inactive; see the notes column.

```{ojs}
//| echo: false
structural_comparison_rows = {
  const alternatives = [
    ["Selected scenario", {}],
    ["Shared maturity", {dependence_mode: "shared"}],
    ["Independent maturity channels", {dependence_mode: "independent_maturity"}],
    ["Density-derived media", {media_use_model: "density"}],
    ["Direct fresh media", {media_use_model: "direct"}],
    ["Separate GF dose prior", {gf_dose_model: "per_kg"}],
    ["Media-linked GF dose", {gf_dose_model: "media_linked"}]
  ];
  return alternatives.map(([scenario, changes]) => {
    const unchanged = Object.entries(changes).every(([key, value]) => results.effective_params[key] === value);
    const r = unchanged ? results : simulate(30000, 42, {...simParams, ...changes});
    return {scenario, p50: quantile(r.unit_cost, 0.5), p90: quantile(r.unit_cost, 0.9),
      under25: 100 * r.unit_cost.filter(c => c < 25).length / r.unit_cost.length,
      notes: r.warnings.join(" ")};
  });
}
html`<div class="structural-comparison-table">
<style>
.structural-comparison-table table { table-layout:fixed; width:100%; border-collapse:collapse; }
.structural-comparison-table th, .structural-comparison-table td { white-space:normal; vertical-align:top; line-height:1.3; padding:.4rem .55rem; border-bottom:1px solid #ddd; text-align:left; }
@media(max-width:600px) { .structural-comparison-table th, .structural-comparison-table td { padding:.35rem .2rem; font-size:.8rem; overflow-wrap:anywhere; } }
</style>
<table>
  <colgroup><col style="width:27%"><col style="width:13%"><col style="width:13%"><col style="width:13%"><col style="width:34%"></colgroup>
  <caption>Same seed and settings; one structural choice changed per row.</caption>
  <thead><tr><th scope="col">Structure</th><th scope="col">p50 ($/kg)</th><th scope="col">p90 ($/kg)</th><th scope="col">P(&lt;$25), %</th><th scope="col">Override notes</th></tr></thead>
  <tbody>${structural_comparison_rows.map(r => html`<tr><th scope="row">${r.scenario}</th><td>${r.p50.toFixed(2)}</td><td>${r.p90.toFixed(2)}</td><td>${r.under25.toFixed(1)}</td><td>${r.notes || "None"}</td></tr>`)}</tbody>
</table></div>`
```

[Why these alternatives, and what remains unmodeled](review-response-2026-09.qmd#structures).
...
- Setting the maturity mean to 0.5 does not remove dependence: maturity still varies across draws. A true independence stress test must change dependence while preserving marginal assumptions.

Use **Structural stress tests → Maturity dependence** to compare shared and independent maturity channels. The explanations below describe the shared baseline. In independent-channel mode the displayed reference maturity distribution is the common marginal distribution; no single maturity draw drives the whole plant scenario.
:::
...
//| echo: false
{
  ...
  if (simParams.dependence_mode !== "shared") return html`<p>The common-maturity scatter plot is omitted: independent-channel mode has no single maturity draw driving total cost.</p>`;
...
This is an assumed dependence structure. Technical success with difficult financing, and the reverse, remain plausible scenarios.
</details>
...
The [versioned September audit](review-2026-09.qmd#what-the-revised-defaults-imply) compares the current defaults across process scenarios and multiple random seeds. Use the live results above for your selected settings. The largest remaining questions concern coherent process productivity and medium requirements, commercial-success conditioning, and evidence for the joint priors.
...
::: {.callout-note collapse="true"}
## Model change log — basal micronutrients folded into media (April 2026) {#micronutrient-change-note}
...
- Supplemental recombinant proteins (insulin, transferrin, albumin) are now broken out as a separate cost line in separable-media mode. Their ranges remain provisional and need supplier quotes or process-linked dosage evidence. In bundled complete-medium mode they are included in media $/L and are not added again.
- Growth factor quantities — **tightened April 2026** from a very broad 0.0001–0.02 g/kg stress-test range to a cited 0.0005–0.006 g/kg default, derived from the Humbird medium formulation reproduced in [GFI 2023](https://gfi.org/wp-content/uploads/2023/01/GFI-report_Anticipated-growth-factor-and-recombinant-protein-costs-and-volumes-necessary-for-cost-competitive-cultivated-meat_2023-1.pdf) (FGF 0.1 mg/L + TGFβ 0.002 mg/L) × media-use assumptions of 8–60 L/kg, and cross-checked against [Pasitka et al. (2024)](https://www.nature.com/articles/s43016-024-01022-w)'s ~0.00187 g/kg empirical ACF figure. Both dosage and price remain uncertain; the current per-kg dosage draw is not coupled to realized medium use.
- Media-use multiplier — **renamed and tightened April 2026** from "Media turnover (1–10×)" to "Media-use multiplier (0.5–3.0×)". The rename flags that the parameter is a bundled ratio (fresh media / nominal reactor volume), not a literal count of reactor-volume changes, so values below 1 (recycling, fed-batch, explicit medium recovery) are now allowed. The new range lets the model represent GFI 2023 cost-competitive scenarios (8–13 L/kg ≈ 0.5–1.2× at 60–90 g/L); the old floor of 1.0 excluded some combinations at fixed density. See [Learn: media-use mechanisms](learn.html#media-use-mechanisms) for the derivation.
